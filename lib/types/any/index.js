    ...
    _validate(value, state, options, reference) {
        ...
        const finish = () => {
            ...
            if (errors.length && typeof this._flags.error === 'function') {
                let change = this._flags.error.call(this, errors);

                if (typeof change === 'string') {
                    change = { message: change };
                }

                let olderr = errors[errors.length - 1];
                errors[errors.length - 1] = this.createOverrideError(olderr.type || 'override', change.context || olderr.context, state, options, change.message || olderr.message, change.template || olderr.template);
                ...
                errors = errors.map((err) => {
                    ...
                    return err instanceof Error ?
                        err :
                        this.createOverrideError(err.type || 'override', change.context || err.context, st	ate, options, change.message || err.message, change.template || err.template);
                });
                */
            }
